...
runner() {
    ...
    python3 $GITDIR/Relation_extraction/preprocess.py $1 > /dev/null 2>&1 
    python3 $GITDIR/Relation_extraction/train_cnn.py $1 > $RE 2> /dev/null

    python3 $GITDIR/sentence_polarity_classification/preprocess.py $1 > /dev/null 2>&1
    python3 $GITDIR/sentence_polarity_classification/train.py $1 > $SPC 2> /dev/null

    python3 $GITDIR/sentiment_classification/train.py $1 > $SC 2> /dev/null
    
    python3 $GITDIR/snli/train.py $1 > $SNLI 2> /dev/null
    
    python3 $GITDIR/subjectivity_classification/preprocess.py $1 > /dev/null 2>&1
    python3 $GITDIR/subjectivity_classification/cnn.py $1 > $SUC 2> /dev/null
